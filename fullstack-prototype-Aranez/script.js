...
function loadFromStorage() {
  ...
  if (!raw) {
    // Missing or first run — seed defaults
    seedDefaults();
    return;
  }
...
function renderVerify() {
  ...
  if (msg && unverifiedEmail) msg.textContent = `Verification sent to ${unverifiedEmail}`;
}
...
function showEmployeeForm(i) {
  // Dynamically populate dept dropdown from current db
...
function renderRequests() {
  ...
  tbody.innerHTML = userRequests.map((req, i) => `
    <tr>
      ...
      <td><button class="btn btn-sm btn-outline-danger" onclick="deleteRequest(${i})">Delete</button></td>
    </tr>
  `).join('');
}
...
function submitRequest() {
  ...
  window.db.requests.push({ userEmail: currentUser.email, type, items });
  saveToStorage();
  requestModal.hide();
  renderRequests();
}
...
function simulateVerification() {
  ...
  if (user) {
    ...
    alert(`Email ${unverifiedEmail} verified! You can now log in.`);
